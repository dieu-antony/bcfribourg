...
const Footer = () => {
  ...
  const { openBanner } = useCookieContext();

  return (
    <footer
      className={`font-sans ${inter.variable} z-30 mt-auto w-full bg-gray-800 p-4 pb-8`}
    >
      <div className="ml-4 mr-4 flex h-full flex-col items-center gap-8 sm:grid sm:grid-cols-3">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="flex flex-row gap-8">
            <Link href="https://www.instagram.com/bcfribourg" target="_blank">
              <svg
            ...
            <Link
              href="https://github.com/dieu-antony/bcfribourg"
              target="_blank"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
        ...
        <button
          onClick={openBanner}
          className="text-gray-500 hover:text-white sm:place-self-start sm:self-center"
        >
          {t("cookieSettings")}
        </button>
        <Link
          className="place-self-center text-gray-500 hover:text-white"
          href="/privacy_policy"
        >
          {t("privacyPolicy")}
        </Link>
        <div className="p-2 text-gray-500 sm:place-self-end sm:self-center">
          &copy; {new Date().getFullYear()} BC Fribourg
        </div>
      </div>
    </footer>
  );
