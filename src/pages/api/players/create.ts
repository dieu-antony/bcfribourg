  ...
  await RouteHandler(req, res, {
    POST: async function (req, res: NextApiResponse) {
      ...
      try {
        ...
        const existingPlayerNames = existingPlayers.map((player) => player.firstName + player.lastName);
        ...
        const playersToCreate = players.filter(
          (player) => !existingPlayerNames.includes(player.firstName + player.lastName)
        );
